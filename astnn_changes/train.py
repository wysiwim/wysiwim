...
if __name__ == '__main__':
    ...
    for t in [4]:#range(1, categories+1):
        ...
        while i < len(test_data_t):
            ...
            truesvals = test_labels.cpu().numpy()
            trues.extend(truesvals)
            total += len(test_labels)
            total_loss += loss.item() * len(test_labels)
            running_corrects += (predicted == truesvals).sum()
        if lang == 'java':
            ...
            accuracy = running_corrects/len(test_data_t)
            if t == 4:
                dataset_name = ""
                experiment_csv_path = ""
                with open("dataset_name.txt", "r") as dnin:
                    dataset_name = dnin.readline()
                with open("experiment_csv_path.txt", "r") as expin:
                    experiment_csv_path = expin.readline()
                resline = "0,astnn,%s,none,%s,%s,%s,%s\n" % (dataset_name, accuracy, p, r, f)
                print(resline)
                with open(experiment_csv_path, "a+") as fout:
                    fout.write(resline)
        else:
